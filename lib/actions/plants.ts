...
export async function identifyPlantAction(
  ...
  // 2) Llamada a Vercel AI Gateway para aprovechar el free tier ($5)
  // y evitar límites de rate de APIs públicas, integrándose con el ecosistema
  // de Vercel. Le pasamos el ID del gateway para que tome la API Key correcta.
  ...
  try {
    ...
    const { output } = await tryModelsInOrder(
      [gateway("google/gemini-2.5-flash"), gateway("google/gemini-2.0-flash")],
      (model) =>
        generateText({
          model,
